...
{
	public class TestContextUser:TestContext
	{

		public TestContextUser(string Domain, string Login, string Password):base(Domain,Login,Password)
		{
		...
		protected override void OnAssertGetCurrentAccount(IePlanifServiceClient Client)
		{
			...
			Assert.AreEqual(account.ProfileID, 2);
		}
		...
		protected override void OnAssertGetCurrentProfile(IePlanifServiceClient Client)
		{
			...
			Assert.AreEqual(profile.AdministrateActivityTypes, false);
			Assert.AreEqual(profile.AdministrateEmployees, false);
			Assert.AreEqual(profile.CanRunReports, false);
		}
